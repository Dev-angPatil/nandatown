...
# ---------------------------------------------------------------------------
# 13. Hardening fixes and enhancements: Replay poisoning, Nonces, and API fit
# ---------------------------------------------------------------------------


class TestReplayCachePoisoning:
    @pytest.mark.asyncio
    async def test_failed_dpop_proof_does_not_poison_replay_cache(self) -> None:
        """Verify that a failed verification due to an invalid/missing DPoP proof
        does NOT prevent a subsequent verification with a valid DPoP proof from succeeding.
        This blocks the replay-cache-poisoning attack."""
        clock = _FakeClock(100.0)
        auth = DpopAuth(secret=b"k", clock=clock)
        agent_pk = b"legit-public-key"
        token = await auth.issue(
            AgentId("a1"), ["read"], audience="svc", bind_to_public_key=agent_pk
        )
        jti = str(_decode_payload(token)["jti"])

        # 1. Attacker tries to present the token without DPoP or with bad DPoP.
        # This must fail verification.
        with pytest.raises(ValueError, match="DPoP-bound"):
            await auth.verify_for_audience(token, audience="svc", dpop=None)

        bad_proof = make_dpop_proof(
            audience="svc", token_jti=jti, iat=clock.t, public_key=b"attacker-pk"
        )
        with pytest.raises(ValueError, match="cnf.jkt"):
            await auth.verify_for_audience(token, audience="svc", dpop=bad_proof)

        # 2. Honest holder presents the token with a valid DPoP proof.
        # Since the replay cache wasn't poisoned by the failed attempts, this MUST succeed!
        good_proof = make_dpop_proof(
            audience="svc", token_jti=jti, iat=clock.t, public_key=agent_pk
        )
        ctx = await auth.verify_for_audience(token, audience="svc", dpop=good_proof)
        assert ctx.subject == AgentId("a1")


class TestServerNonce:
    @pytest.mark.asyncio
    async def test_nonce_success(self) -> None:
        clock = _FakeClock(100.0)
        auth = DpopAuth(secret=b"k", clock=clock)
        agent_pk = b"legit-public-key"
        token = await auth.issue(
        ...
        # Verifier issues a server nonce
        nonce = auth.issue_dpop_nonce("svc")

        # Client builds a proof echoing the nonce
        proof = make_dpop_proof(
            audience="svc", token_jti=jti, iat=clock.t, public_key=agent_pk, nonce=nonce
        )
    ...
    @pytest.mark.asyncio
    async def test_nonce_single_use(self) -> None:
        clock = _FakeClock(100.0)
        auth = DpopAuth(secret=b"k", clock=clock)
        agent_pk = b"legit-public-key"
        token = await auth.issue(
        ...
        # Verification succeeds first time
        await auth.verify_for_audience(token, audience="svc", dpop=proof)

        # A second attempt with the same nonce fails because it is single-use and was consumed
        # (Even if we bypass replay cache, the nonce validation itself will fail)
        token2 = await auth.issue(
            AgentId("a1"), ["read"], audience="svc", bind_to_public_key=agent_pk
        )
        jti2 = str(_decode_payload(token2)["jti"])
        proof2 = make_dpop_proof(
            audience="svc", token_jti=jti2, iat=clock.t, public_key=agent_pk, nonce=nonce
        )
        with pytest.raises(ValueError, match="Invalid or expired DPoP server nonce"):
            await auth.verify_for_audience(token2, audience="svc", dpop=proof2)

    @pytest.mark.asyncio
    async def test_missing_required_nonce_fails(self) -> None:
        clock = _FakeClock(100.0)
        auth = DpopAuth(secret=b"k", clock=clock)
        agent_pk = b"legit-public-key"
        token = await auth.issue(
        ...
        # Verifier issues a server nonce (making it required)
        _nonce = auth.issue_dpop_nonce("svc")

        # Client presents a proof without nonce
        proof = make_dpop_proof(
            audience="svc", token_jti=jti, iat=clock.t, public_key=agent_pk, nonce=None
        )
        with pytest.raises(ValueError, match="DPoP proof missing required server nonce"):
            await auth.verify_for_audience(token, audience="svc", dpop=proof)

    @pytest.mark.asyncio
    async def test_wrong_nonce_fails(self) -> None:
        clock = _FakeClock(100.0)
        auth = DpopAuth(secret=b"k", clock=clock)
        agent_pk = b"legit-public-key"
        token = await auth.issue(
        ...
        # Client presents a proof with a wrong nonce
        proof = make_dpop_proof(
            audience="svc", token_jti=jti, iat=clock.t, public_key=agent_pk, nonce="wrong-nonce"
        )
        with pytest.raises(ValueError, match="Invalid or expired DPoP server nonce"):
            await auth.verify_for_audience(token, audience="svc", dpop=proof)
...
class TestSecureByDefault:
    @pytest.mark.asyncio
    async def test_verify_bare_rejects_bound_tokens(self) -> None:
        """Calling verify(token) bare (no kwargs) on audience/cnf bound tokens
        must fail, preventing drop-in users from ignoring the bounds."""
        auth = DpopAuth(secret=b"k")

        # Token with audience bound
        aud_token = await auth.issue(AgentId("a1"), ["read"], audience="svc")
        with pytest.raises(ValueError, match="Token requires audience verification"):
            await auth.verify(aud_token)

        # Token with DPoP bound
        dpop_token = await auth.issue(AgentId("a1"), ["read"], bind_to_public_key=b"pk")
        with pytest.raises(ValueError, match="Token is DPoP-bound"):
            await auth.verify(dpop_token)

    @pytest.mark.asyncio
    async def test_verify_with_kwargs_hardened(self) -> None:
        """Calling verify(token, audience=..., dpop=...) must support the optional kwargs
        and correctly validate the claims."""
        clock = _FakeClock(100.0)
        auth = DpopAuth(secret=b"k", clock=clock)
        agent_pk = b"pk"
        token = await auth.issue(
        ...
        ctx = await auth.verify(token, audience="svc", dpop=proof)
        assert ctx.subject == AgentId("a1")
...
class TestPropertyBased:
    @given(
        subject_id=st.text(min_size=1, max_size=50).filter(lambda x: ":" not in x),
        scopes=st.lists(st.text(min_size=1, max_size=20), min_size=0, max_size=5),
        audience=st.one_of(st.none(), st.text(min_size=1, max_size=50)),
        ttl=st.floats(min_value=1.0, max_value=10000.0),
    )
    @pytest.mark.asyncio
    async def test_token_properties(
        self, subject_id: str, scopes: list[str], audience: str | None, ttl: float
    ) -> None:
        clock = _FakeClock(100.0)
        auth = DpopAuth(secret=b"prop-test-secret", clock=clock, token_ttl_seconds=ttl)
        subject = AgentId(subject_id)
        token = await auth.issue(subject, scopes, audience=audience)

        # Decode & inspect payload
        payload = _decode_payload(token)
        assert payload["sub"] == subject_id
        assert payload["scopes"] == scopes
        assert payload["iat"] == 100.0
        assert payload["exp"] == 100.0 + ttl
        ...
        if audience is not None:
            ...
            with pytest.raises(ValueError):
                await auth.verify(token)
            # Audience verify succeeds
            ctx = await auth.verify(token, audience=audience)
            assert ctx.subject == subject
            assert ctx.scopes == scopes
        else:
            assert "aud" not in payload
            # Bare verify succeeds
            ctx = await auth.verify(token)
            assert ctx.subject == subject
            assert ctx.scopes == scopes
