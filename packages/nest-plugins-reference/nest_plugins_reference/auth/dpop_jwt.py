...
class DpopProof:
    ...
    Example::
        ...
        proof = DpopProof(jti="abc", audience="payments", iat=100.0, signature=b"...", nonce="x")
    """
...
class DpopAuth:
    ...
    async def verify(
        ...
        *,
        ...
        Supports optional audience and DPoP proof parameters for drop-in hardening
        while maintaining compatibility with the base Auth protocol.
        """
    ...
    def _verify_core(
        ...
        *,
        ...
        if token_aud is not None and audience is None:
            msg = "Token requires audience verification"
            raise ValueError(msg)
        if audience is not None:
        ...
        # prove possession of the key.  This is checked BEFORE the replay
        # cache is updated to prevent replay-cache poisoning attacks.
        cnf_raw: object = payload.get("cnf")
        jkt_val = None
        if isinstance(cnf_raw, dict):
            jkt_val = cast("dict[str, Any]", cnf_raw).get("jkt")
        ...
        # audience, and DPoP validation so attackers cannot use this path to
        # poison the cache with arbitrary/invalid tokens.
        if audience is not None:
    ...
    def _verify_dpop_proof(
        ...
        *,
        ...
        # 5. Nonce verification if verifier has issued nonces for this audience.
        expected_nonce = proof.nonce
        if audience in self._nonces:
            if expected_nonce is None:
                msg = "DPoP proof missing required server nonce"
                raise ValueError(msg)
            if expected_nonce not in self._nonces[audience]:
                msg = "Invalid or expired DPoP server nonce"
                raise ValueError(msg)
            # Consume the nonce (single-use)
            self._nonces[audience].remove(expected_nonce)

        # 6. Verify the proof signature.  We accept either:
        ...
        signing_input = self.build_dpop_signing_input(
            audience=audience, token_jti=token_jti, iat=proof.iat, nonce=proof.nonce
        )
    ...
    *,
    ...
    Example::
        ...
        proof = make_dpop_proof(
            audience="svc", token_jti="abc", iat=1.0,
            public_key=b"k", nonce="123",
        )
    """
    signing_input = DpopAuth.build_dpop_signing_input(
        audience=audience, token_jti=token_jti, iat=iat, nonce=nonce
    )
